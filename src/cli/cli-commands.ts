  ...
  .action(async (input: string, output: string, cmdOptions: any, command: Command) => {
    try {
      ...
      const options: CLIOptions = {
        ...
        debug: globalOptions.debug || process.env.DEBUG === 'true',
        quiet: globalOptions.quiet || process.env.QUIET === 'true',
        openaiApiKey: globalOptions.openaiApiKey || process.env.OPENAI_API_KEY,
        model: globalOptions.model || process.env.OPENAI_MODEL || 'gpt-4',
        noAi: globalOptions.noAi || process.env.NO_AI === 'true',
