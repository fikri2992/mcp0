...
exports.testCompleteFlow = testCompleteFlow;
...
async function testCompleteFlow() {
    ...
    for (const test of flowTests) {
        ...
        try {
            ...
            const validatedParams = server.requestValidator.validateToolCall(test.toolName, test.params);
            ...
            const apiRequest = server.convertToAPIRequest(test.toolName, validatedParams);
            // Step 3: Validate API request
            const validatedRequest = server.requestValidator.validateAPIRequest(apiRequest);
            ...
            switch (test.toolName) {
                case 'api_get':
                    apiResponse = await server.apiClient.get(validatedRequest.url, validatedRequest.headers);
                    break;
                case 'api_post':
                    apiResponse = await server.apiClient.post(validatedRequest.url, validatedRequest.body, validatedRequest.headers);
                    break;
                case 'api_put':
                    apiResponse = await server.apiClient.put(validatedRequest.url, validatedRequest.body, validatedRequest.headers);
                    break;
                case 'api_delete':
                    apiResponse = await server.apiClient.delete(validatedRequest.url, validatedRequest.headers);
                    break;
                default:
            ...
            const formattedResponse = server.responseFormatter.formatResponse(apiResponse);
...
//# sourceMappingURL=complete-flow.test.js.map