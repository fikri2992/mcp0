...
exports.testResponseTypes = testResponseTypes;
...
async function testResponseTypes() {
    ...
    for (const test of tests) {
        ...
        try {
            ...
            const result = await server.handleToolCall(test.toolName, test.params);
            if ('error' in result) {
                console.log(`❌ Tool call failed: ${result.error.message}`);
                continue;
            }
            // Format the response
            const formatted = server.responseFormatter.formatResponse(result);
...
//# sourceMappingURL=response-types.test.js.map